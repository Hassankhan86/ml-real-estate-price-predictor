...
if st.button('Search'):
    ...
    appartment = [
        key for key, value in result_ser.items()
    ]

    st.session_state.appartment_list = appartment


# Show radio if search was done
if "appartment_list" in st.session_state:

    def update_selectbox():
        # When radio changes, update selectbox value
        st.session_state.selected_appartment = st.session_state.radio_choice

    st.radio(
        "Select anyone for recommendation",
        st.session_state.appartment_list,
        key="radio_choice",
